...
// prepare input field
var w = 250E-6;
var sfx = new Samp1DCplxFunc(f: (x) => Function1D.Gaussian(x, new List<double>{w}));
var vIn = sfx.Sample(grid: gx, loopMode: LoopMode.Parallel);
VFrame.CreateShow(values: VMath.Abs(vIn), title: $"Input E-field [Magnitude]",
    grid: gx * 1E3, // scaled to [mm] unit
    xLabel: $"x [mm]", yLabel: $"value [A.U.]");


// FT-BPM
var nLayers = 29;
var bpm = new FFTBPM1D(mediumX: med);
var vOut = bpm.Propagate(
    wavelength: wavelength,
    vIn: vIn, gIn: gx,
    zStart: 0.0, zEnd: d, nLayers: nLayers,
    out List<VectorZ> vInside,
    n0: 2.0,
