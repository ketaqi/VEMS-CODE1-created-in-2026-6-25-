...
public class RibbonTab : TabItem, IKeyTipHandler
{
    public static readonly DirectProperty<RibbonTab, ObservableCollection<RibbonGroupBox>> GroupsProperty =
        AvaloniaProperty.RegisterDirect<RibbonTab, ObservableCollection<RibbonGroupBox>>(nameof(Groups), o => o.Groups, (o, v) => o.Groups = v);

    public static readonly StyledProperty<bool> IsContextualProperty = AvaloniaProperty.Register<RibbonTab, bool>(nameof(IsContextual));
    ...
    public RibbonTab()
    {
        LostFocus += (_, _) => KeyTip.SetShowChildKeyTipKeys(this, false);
    }

    protected override Type StyleKeyOverride { get; } = typeof(RibbonTab);
            ...
            else
            {
                foreach (Control c in g.Items)
                    if (KeyTip.HasKeyTipKey(c, key))
                    {
                        ...
                        {
                            if (c is Button { Command: not null } btn)
    ...
    {
        KeyTip.SetShowChildKeyTipKeys(this, false);
        this.FindAncestorOfType<Ribbon>(true)?.Close();
    }
}