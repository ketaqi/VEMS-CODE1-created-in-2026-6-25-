...
/// <summary>
/// <see cref="IExecutionHost"/> 的实现，将代码编译到磁盘并在独立进程中执行。
/// </summary>
/// <remarks>
/// 支持两种编译模式：
/// <list type="bullet">
/// <item><description>脚本模式（.csx）：进程内编译</description></item>
/// <item><description>常规模式（.cs）：通过 MSBuild 编译</description></item>
/// </list>
/// 执行时会启动子进程运行编译后的程序集，通过标准输入/输出进行 JSON 格式的结果通信。
/// </remarks>
internal partial class ExecutionHost : IExecutionHost, IDisposable
{
    ...
    /// 子进程启动时触发的事件。
    /// </summary>
    internal event Action<int>? ProcessStarted;

    /// <summary>
    /// 获取或设置执行平台。设置时会初始化构建路径。
    /// </summary>
    ...
    /// <summary>
    /// 初始化 <see cref="ExecutionHost"/> 类的新实例。
    /// </summary>
    /// <param name="parameters">执行宿主参数。</param>
    /// <param name="roslynHost">Roslyn 宿主实例。</param>
    ...
    /// 初始化构建路径。
    /// </summary>
    /// <param name="stopProcess">是否停止正在运行的进程。</param>
    ...
    /// <param name="disassemble">是否输出反汇编结果。</param>
    ...
    private async Task ExecuteAssemblyAsync(string assemblyPath, CancellationToken cancellationToken)
    {
        ...
        try { ProcessStarted?.Invoke(_childProcessId); } catch { }
    ...
    /// </summary>
    /// <remarks>
    /// 转换包括：移除 #r 引用指令、为最后一个无分号的表达式添加 .Dump() 调用。
    /// </remarks>
